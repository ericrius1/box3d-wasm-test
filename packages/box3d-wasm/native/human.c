...
// Local stand-in for the samples-app RandomVec3 (utils.h is not vendored).
static uint32_t s_humanRandom = 0x9e3779b9u;
static float HumanRandomFloat( float lo, float hi )
{
	s_humanRandom = s_humanRandom * 1664525u + 1013904223u;
	float t = (float)( s_humanRandom >> 8 ) / 16777216.0f;
	return lo + t * ( hi - lo );
}

static b3Vec3 RandomVec3( b3Vec3 lo, b3Vec3 hi )
{
	return (b3Vec3){ HumanRandomFloat( lo.x, hi.x ), HumanRandomFloat( lo.y, hi.y ), HumanRandomFloat( lo.z, hi.z ) };
}
